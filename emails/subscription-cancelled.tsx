...
export function subscriptionCancelledEmail({
  ...
  plan_label,
  cancel_at,
}: SubscriptionCancelledProps): { subject: string; html: string } {
  const fullName = `${prenom} ${nom}`.trim()
  const dateStr = formatDateFr(cancel_at)
  const subject = `Confirmation de résiliation — NoX VTC`
          ...
          <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:560px;background:#1A1A1A;border:1px solid #333;border-radius:16px;padding:32px;">
            ...
            <tr>
              <td style="color:#F5F5F5;font-size:16px;line-height:1.6;">
                ...
                <p style="margin:0 0 16px;">Votre résiliation a bien été prise en compte.</p>
                <p style="margin:0 0 8px;">Votre accès <strong style="color:#C9A84C;">${plan_label}</strong> reste actif jusqu'au <strong style="color:#F5F5F5;">${dateStr}</strong>.</p>
                <p style="margin:0 0 8px;">À cette date, votre compte basculera automatiquement en offre <strong>Starter</strong> (gratuite).</p>
