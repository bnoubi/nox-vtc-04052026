...
export function QuickAddClientModal({ open, onClose, clients, onClientCreated }: QuickAddClientModalProps) {
  ...
  const canSubmit =
    ...
    nom.trim().length > 0 &&
    phone.trim().length > 0 &&
    (isParticulier || !!selectedSociete || societeSearch.trim().length > 0)
            ...
            <form id="quick-add-form" onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto flex-1 min-h-0">
                ...
                <div className="flex gap-2">
                  {([
                    ...
                    <button
                      key={t.value}
                      type="button"
                      onClick={() => { setClientType(t.value); setSelectedSociete(null); setSocieteSearch("") }}
                      className={cn(
              ...
              {!isParticulier && (
                <div>
                  ...
                  {selectedSociete ? (
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="text"
                        value={societeSearch}
                        onChange={e => setSocieteSearch(e.target.value)}
                        onFocus={() => setSocieteFocused(true)}
                        onBlur={() => setTimeout(() => setSocieteFocused(false), 200)}
                        placeholder="Rechercher une société existante..."
                        className="w-full pl-10 pr-4 py-3 rounded-xl bg-onyx-card border border-onyx-border/50 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-gold/40 transition-colors"
                        style={{ fontSize: "16px" }}
                      />
                  ...
                  {!selectedSociete && societeSearch.trim().length === 0 && (
